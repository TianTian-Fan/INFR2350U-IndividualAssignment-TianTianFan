...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		ShaderProgram::Sptr discard = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/discard.glsl" }
		});

		discard->SetDebugName("Discard");

		// Basic gbuffer generation with no vertex manipulation
		ShaderProgram::Sptr blingPhong = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/frag_blinn_phong_textured.glsl" }
		});
		blingPhong->SetDebugName("Bling");
		...
		Texture2D::Sptr    discardBox   = ResourceManager::CreateAsset<Texture2D>("textures/box3.bmp");
...
#pragma endregion 
		...
		Texture3D::Sptr cus = ResourceManager::CreateAsset<Texture3D>("luts/Cust.CUBE");
		...
		Material::Sptr discardMaterial = ResourceManager::CreateAsset<Material>(discard);
		{
			discardMaterial->Name = "DiscardBox";
			discardMaterial->Set("u_Material.Diffuse", discardBox);
			discardMaterial->Set("u_Material.Shininess", 1.0f);
			discardMaterial->Set("u_Material.NormalMap", normalMapDefault);
		}


		Material::Sptr boxMaterial = ResourceManager::CreateAsset<Material>(blingPhong);
		{
		...
		Material::Sptr monkeyMaterial = ResourceManager::CreateAsset<Material>(blingPhong);
		{
		...
		Material::Sptr testMaterial = ResourceManager::CreateAsset<Material>(blingPhong); 
		{
		...
		/*Material::Sptr foliageMaterial = ResourceManager::CreateAsset<Material>(foliageShader);
		{
			...
			foliageMaterial->Set("u_WindStrength", 1.0f);
			foliageMaterial->Set("u_VerticalScale", 1.0f);
			foliageMaterial->Set("u_WindSpeed", 2.0f);
		}
		...
		for (int ix = 0; ix < 4; ix++) {
			...
			lightComponent->SetIntensity(glm::linearRand(2.0f, 4.0f));
		}*/
		...
		lightComponent->SetRadius(glm::linearRand(20.0f, 20.0f));
		lightComponent->SetIntensity(glm::linearRand(200.0f, 201.0f));

		GameObject::Sptr light2 = scene->CreateGameObject("Light2");
		light2->SetPostion(glm::vec3(4.5f, -1.0f, 1.0f));

		Light::Sptr lightComponent1 = light2->Add<Light>();
		lightComponent1->SetColor(glm::vec3(1.0f, 1.0f, 1.0f));
		lightComponent1->SetRadius(glm::linearRand(20.0f, 20.0f));
		lightComponent1->SetIntensity(glm::linearRand(200.0f, 201.0f));

		GameObject::Sptr light3 = scene->CreateGameObject("Light3");
		light3->SetPostion(glm::vec3(0.2f, 2.5f, 1.0f));

		Light::Sptr lightComponent2 = light3->Add<Light>();
		lightComponent2->SetColor(glm::vec3(1.0f, 1.0f, 1.0f));
		lightComponent2->SetRadius(glm::linearRand(20.0f, 20.0f));
		lightComponent2->SetIntensity(glm::linearRand(200.0f, 201.0f));

		GameObject::Sptr light4 = scene->CreateGameObject("Light4");
		light4->SetPostion(glm::vec3(0.0f, -2.0f, 0.6f));

		Light::Sptr lightComponent3 = light4->Add<Light>();
		lightComponent3->SetColor(glm::vec3(1.0f, 1.0f, 1.0f));
		lightComponent3->SetRadius(glm::linearRand(20.0f, 20.0f));
		lightComponent3->SetIntensity(glm::linearRand(200.0f, 201.0f));

		GameObject::Sptr light5 = scene->CreateGameObject("Light5");
		light5->SetPostion(glm::vec3(5.4f, 3.0f, 1.0f));

		Light::Sptr lightComponent4 = light5->Add<Light>();
		lightComponent4->SetColor(glm::vec3(1.0f, 1.0f, 1.0f));
		lightComponent4->SetRadius(glm::linearRand(-150.0f, -150.0f));
		lightComponent4->SetIntensity(glm::linearRand(200.0f, 201.0f));
		...
		GameObject::Sptr DiscardBox = scene->CreateGameObject("discardBox");
		{
			DiscardBox->SetPostion(glm::vec3(1.6f, 0.0f, 1.0f));
			DiscardBox->SetScale(glm::vec3(3.0f, 3.0f, 2.0f));
			...
			RenderComponent::Sptr renderer = DiscardBox->Add<RenderComponent>();
			renderer->SetMesh(boxMesh);
			renderer->SetMaterial(discardMaterial);
		}
		...
		/*GameObject::Sptr foliageBox = scene->CreateGameObject("Foliage Box");
		{
			...
			// Set and rotation position in the scene
			foliageBox->SetPostion(glm::vec3(-3.5f, -0.0f, 0.5f));
			foliageBox->SetScale(glm::vec3(2.0f, 6.0f, 1.0f));
